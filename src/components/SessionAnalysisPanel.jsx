...
import { AlertTriangle, Brain, ChevronDown, History, Play } from 'lucide-react';
import {
...
export const SessionAnalysisPanel = ({
  ...
  const sessionAnalysisHistoryStatusById = useSelector((state) => state.poker.sessionAnalysisHistoryStatusById);
  const sessionAnalysisHistoryErrorById = useSelector((state) => state.poker.sessionAnalysisHistoryErrorById);
  ...
  const historyStatus = sessionAnalysisHistoryStatusById[sessionId] || 'idle';
  const historyError = sessionAnalysisHistoryErrorById[sessionId];
  ...
  const panelId = `session-analysis-content-${sessionId}`;

  const isExpanded = expanded || Boolean(selectedReportId);

  useEffect(() => {
    if (!isExpanded) return;
    dispatch(fetchSessionAnalysisHistory({ sessionId }));
  }, [dispatch, historyStatus, isExpanded, sessionId]);

  useEffect(() => () => {
    dispatch(releaseSessionAnalysisHistory(sessionId));
  }, [dispatch, sessionId]);
  ...
  return (
    <section data-testid="session-analysis-panel" className={`rounded-xl border ${accentClasses}`}>
      <button
        type="button"
        aria-expanded={isExpanded}
        aria-controls={panelId}
        onClick={toggleExpanded}
        className="flex w-full items-center justify-between gap-3 p-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-indigo-500"
      >
        <span className="min-w-0">
          <span className="flex items-center gap-2 font-black"><Brain size={18}/> Analiza AI sesji</span>
          <span className="mt-1 block text-xs opacity-75">Rozwiń, aby zobaczyć raport i historię analiz dla {handCount} rozdań.</span>
        </span>
        <span className="flex shrink-0 items-center gap-2">
          ...
          <ChevronDown aria-hidden="true" size={18} className={`transition-transform ${isExpanded ? 'rotate-180' : ''}`}/>
        </span>
      </button>

      {isExpanded && <div id={panelId} className="border-t border-current/10 px-4 pb-4">
        {historyStatus === 'loading' && history.length === 0 && <div className="mt-4 flex items-center gap-3 rounded-lg bg-white/75 p-4 text-sm font-bold"><span className="h-5 w-5 animate-spin rounded-full border-2 border-current border-t-transparent"/> Wczytywanie historii raportów…</div>}
        {historyStatus === 'failed' && <div role="alert" className="mt-4 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700"><p>{typeof historyError === 'string' ? historyError : historyError?.message}</p><button type="button" onClick={() => dispatch(releaseSessionAnalysisHistory(sessionId))} className="mt-3 rounded-lg border border-red-200 bg-white px-3 py-2 text-xs font-black text-red-700">Spróbuj ponownie</button></div>}
        {history.length > 0 && (
